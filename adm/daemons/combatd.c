...
//	return "造成 " + damage + " 点" + type + "。\n";
	...
	case "内伤":
		...
		break;
	case "抓伤":
		if( damage < 10 ) return "结果只是轻轻地抓破$p的皮肉。\n";
		else if( damage < 20 ) return "结果在$p$l抓出一道细长血痕。\n";
		else if( damage < 40 ) return "结果抓出一道长长的血口！\n";
		else if( damage < 80 ) return "结果抓出一道血淋淋的伤口！\n";
		else if( damage < 160 ) return "结果抓出一道又长又深的伤口，$N满手沾满了鲜血！\n";
		else return "结果只听见$n一声惨嚎，$p$l被抓出一道深及见骨的可怕伤口！！\n";
		break;
	default:
		if( !type ) type = "伤害";
...
/* 招架的兵器 体现在parry.c中 */
	...
	else result = replace_string( result, "$W", "拳脚" );
	if( objectp(weapon) )
...
// Give the death penalty to the dying user.
      ...
      msg1+=killer->name(1);
			...
			{
				case "擦伤":
				case "割伤": msg+="砍死了";msg1+="砍死了"; break;
				case "刺伤": msg+="刺死了";msg1+="刺死了"; break;
				case "瘀伤": msg+="击死了";msg1+="击死了"; break;
				case "内伤": msg+="震死了";msg1+="震死了"; break;
				default: msg+="杀死了";msg1+="杀死了";
			}
		}
