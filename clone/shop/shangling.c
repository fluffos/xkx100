...
int do_stock(string arg)
{
	...
	if (! arg) return notify_fail("指令格式：stock <货物> <原值倍数>\n");
	...
	if (goods->is_character()) return notify_fail("你不能贩卖活物。\n");
...
//	if (goods->is_item_make())
	...
	if (! value) return notify_fail("这东西不值钱，没人会去买的。\n");
...
int do_unstock(string arg)
{
	...
	all_goods = me->query("vendor_goods");
	if (! all_goods) return notify_fail("你现在并没有兜售任何货物。\n");
	...
	else return notify_fail("你并没有兜售这个货物。\n");
